...
public class MyMovieDBDetailReviewsAsync extends AsyncTask<String, Void, Reviews> {

    private static final String LOG = MyMovieDBDetailReviewsAsync.class.getName();

    private final Context contextoMyMovieDBDetail;

    public MyMovieDBDetailReviewsAsync(Context context) {
    ...
    protected Reviews doInBackground(String... params) {
        ...
        StringBuilder sbReviewsMovies = obtenerPeliculasDeIMDB(uri);
        if (null != sbReviewsMovies) {
            return obtenerListaReviewsMovie(sbReviewsMovies);
        }else{
            return null;
        }
    }
    ...
    protected void onPostExecute(Reviews s) {
        ((DetailActivityMyMovieDB) contextoMyMovieDBDetail).getReviewAdapter().setReviews(s);
        ((DetailActivityMyMovieDB) contextoMyMovieDBDetail).getReviewAdapter().notifyDataSetChanged();
    }
    ...
    private Reviews obtenerListaReviewsMovie(StringBuilder jsonPelis) {
        Log.i(LOG, jsonPelis.toString());
        Gson gSon=  new GsonBuilder().setDateFormat(
                contextoMyMovieDBDetail.getString(R.string.formato_fecha)).create();
        return gSon.fromJson(jsonPelis.toString(), Reviews.class);
    }
    ...
    private Uri construirUri(String[] params) {
        Uri uri = new Uri.Builder()
                ...
                .appendPath(contextoMyMovieDBDetail.getString(R.string.url_imdb_reviews))
